...
#   panel, with explicit rolls when the front symbol changes.
# - Commissions and financing are generated by `IBKRProFixedBroker`.
# - USD financing rates use `data/ibkr_usd_benchmark_schedule_2025.csv` as a
...
voo_df = load_voo_total_return_df(
    voo_ohlcv_path,
    voo_dividends_path,
    voo_splits_path;
...
mes_front_df = load_mes_front_df(
    mes_contracts_path;
    start_dt=BACKTEST_START,
    end_dt=BACKTEST_END,
...
@assert nrow(voo_df) == nrow(mes_front_df) && nrow(voo_df) > 0

## MES rolling and fee setup derived from observed front-contract switches
mes_contract_specs = build_mes_contract_specs(mes_front_df)
